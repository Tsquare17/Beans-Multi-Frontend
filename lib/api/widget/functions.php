...
namespace Beans\Framework\API\Widgets;
...
function beans_register_widget_area( $args = array() ) {
	...
	$id = Utilities\beans_get( 'id', $args );
	 ...
	 * @since 1.0.0
	 */
	$args = Filters\beans_apply_filters( "beans_widgets_area_args[_{$id}]", array_merge( $defaults, $args ) );
...
function beans_get_widget_area( $needle = false ) {
	...
	return Utilities\beans_get( $needle, $_beans_widget_area, false );
}
...
function beans_widget_area_shortcodes( $content ) {
	...
	return Utilities\beans_array_shortcodes( $content, $GLOBALS['_beans_widget_area'] );
}
...
function beans_have_widgets() {
	...
	if ( ! Utilities\beans_get( 'widgets', $_beans_widget_area ) ) {
		return false;
	}
...
function beans_setup_widget() {
	...
	$id = Utilities\beans_get( $_beans_widget_area['current_widget'], $widgets );
...
function beans_get_widget( $needle = false ) {
	...
	return Utilities\beans_get( $needle, $_beans_widget, false );
}
...
function beans_widget_shortcodes( $content ) {
	...
	return Utilities\beans_array_shortcodes( $content, $GLOBALS['_beans_widget'] );
}
...
function _beans_setup_widget_area( $id ) {
	...
	$sidebar = Utilities\beans_render_function( 'dynamic_sidebar', $id );
...
function _beans_setup_widgets( $widget_area_content ) {
	...
	foreach ( explode( '<!--widget-end-->', $widget_area_content ) as $content ) {
		...
		$data = Utilities\beans_get( $id, $wp_registered_widgets );
...
function _beans_force_the_widget( $widget, $instance, $args ) {
	...
	if ( false !== stripos( Utilities\beans_get( 'before_widget', $args ), $widget_obj->id ) ) {
		return;
	}
