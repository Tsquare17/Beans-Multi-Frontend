...
function beans_register_options( array $fields, $menu_slug, $section, $args = array() ) {
	 ...
	 */
	$fields = apply_filters( "beans_options_fields_{$section}", Fields\_beans_pre_standardize_fields( $fields ) );
	...
	if ( ( Utilities\beans_get( 'page' ) !== $menu_slug ) || ! is_admin() ) {
		return;
	}

	// Stop here if the field can't be registered.
	if ( ! Fields\beans_register_fields( $fields, 'option', $section ) ) {
		return false;
	}
...
function beans_options( $menu_slug ) {
	if ( ! class_exists( __NAMESPACE__ . '\_Beans_Options' ) ) {
		return false;
	}
...
add_action( 'wp_loaded', __NAMESPACE__ . '\_beans_options_page_actions' );
/**
...
function _beans_options_page_actions() {
	...
	if ( ! Utilities\beans_post( 'beans_options_nonce' ) ) {
		return;
	}
