...
final class _Beans_Options {
	...
	public function render_metabox() {
		$fields = Fields\beans_get_fields( 'option', $this->section );
	...
	public function render_page( $page ) {
		...
		$boxes = Utilities\beans_get( $page, $wp_meta_boxes );
		...
		$column_class = Utilities\beans_get( 'column', $boxes, array() ) ? ' column' : false;
	...
	public function process_actions() {
		...
		if ( Utilities\beans_post( 'beans_save_options' ) ) {
			$this->save();
			add_action( 'admin_notices', array( $this, 'render_save_notice' ) );
		}

		if ( Utilities\beans_post( 'beans_reset_options' ) ) {
			$this->reset();
	...
	private function save() {
		...
		if ( ! wp_verify_nonce( Utilities\beans_post( 'beans_options_nonce' ), 'beans_options_nonce' ) ) {
			return false;
		}
