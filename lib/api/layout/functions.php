...
namespace Beans\Framework\API\Layout;
...
function beans_get_default_layout() {
	$default_layout = Widgets\beans_has_widget_area( 'sidebar_primary' ) ? 'c_sp' : 'c';
...
function beans_get_layout() {
	...
	if ( is_singular() ) {
		...
		$layout = Term_Meta\beans_get_term_meta( 'beans_layout' );
	}
...
function beans_get_layout_class( $id ) {
	 ...
	 */
	return apply_filters( "beans_layout_class_{$id}", Utilities\beans_get( $id, _beans_get_layout_classes( $args ) ) );
}
...
function _beans_get_layout_classes( array $args ) {
	$grid   = Utilities\beans_get( 'grid', $args );
	$c      = $grid; // $c stands for "content".
	$sp     = Utilities\beans_get( 'sidebar_primary', $args );
	$ss     = Utilities\beans_get( 'sidebar_secondary', $args );
	$prefix = 'uk-width-' . Utilities\beans_get( 'breakpoint', $args, 'medium' );

	$classes = array(
		'content' => "{$prefix}-{$c}-{$grid}",
	);

	if ( ! Widgets\beans_has_widget_area( 'sidebar_primary' ) ) {
		return $classes;
	}

	$layout        = beans_get_layout();
	$has_secondary = Widgets\beans_has_widget_area( 'sidebar_secondary' );
...
function beans_get_layouts_for_options( $add_default = false ) {
	...
	$layouts = array(
		'c' => array(
			'src'                => $base . 'c.png',
			'alt'                => __( 'Full-Width Content Layout', 'beans' ),
			'screen_reader_text' => __( 'Option for the Full-Width Content Layout.', 'beans' ),
		),
	);

	// Add sidebar primary layouts if the primary widget area is registered.
	$has_primary = Widgets\beans_has_widget_area( 'sidebar_primary' );

	if ( $has_primary ) {
		$layouts['c_sp']['src']                = $base . 'cs.png';
		$layouts['c_sp']['alt']                = __( 'Content and Primary Sidebar Layout', 'beans' );
		$layouts['c_sp']['screen_reader_text'] = __( 'Option for the Content and Primary Sidebar Layout.', 'beans' );

		$layouts['sp_c']['src']                = $base . 'sc.png';
		$layouts['sp_c']['alt']                = __( 'Primary Sidebar and Content Layout', 'beans' );
		$layouts['sp_c']['screen_reader_text'] = __( 'Option for the Primary Sidebar and Content Layout.', 'beans' );
	}

	// Add sidebar secondary layouts if the primary and secondary widget area are registered.
	if ( $has_primary && Widgets\beans_has_widget_area( 'sidebar_secondary' ) ) {
		$layouts['c_sp_ss']['src']                = $base . 'css.png';
		$layouts['c_sp_ss']['alt']                = __( 'Content, Primary Sidebar and Secondary Sidebar Layout', 'beans' );
		$layouts['c_sp_ss']['screen_reader_text'] = __( 'Option for the Content, Primary Sidebar and Secondary Sidebar Layout.', 'beans' );

		$layouts['sp_ss_c']['src']                = $base . 'ssc.png';
		$layouts['sp_ss_c']['alt']                = __( 'Primary Sidebar, Secondary Sidebar and Content Layout', 'beans' );
		$layouts['sp_ss_c']['screen_reader_text'] = __( 'Option for the Primary Sidebar, Secondary Sidebar and Content Layout.', 'beans' );

		$layouts['sp_c_ss']['src']                = $base . 'scs.png';
		$layouts['sp_c_ss']['alt']                = __( 'Primary Sidebar, Content and Secondary Sidebar Layout', 'beans' );
		$layouts['sp_c_ss']['screen_reader_text'] = __( 'Option for the Primary Sidebar, Content and Secondary Sidebar Layout.', 'beans' );
	}
...
function beans_has_primary_sidebar( $layout ) {
	...
	return Widgets\beans_is_active_widget_area( 'sidebar_primary' );
}
...
function beans_has_secondary_sidebar( $layout ) {
	...
	return Widgets\beans_is_active_widget_area( 'sidebar_secondary' );
}
