...
Actions\beans_add_smart_action( 'beans_field_enqueue_scripts_image', __NAMESPACE__ . '\beans_field_image_assets' );
/**
...
Actions\beans_add_smart_action( 'beans_field_image', 'beans_field_image' );
/**
...
function beans_field_image( array $field ) {
	...
	$is_multiple = Utilities\beans_get( 'multiple', $field );
	$link_text   = _n( 'Add Image', 'Add Images', ( $is_multiple ? 2 : 1 ), 'beans' );
...
function _beans_get_image_url( $image_id ) {
	...
	return Utilities\beans_get( 0, wp_get_attachment_image_src( $image_id, 'thumbnail' ) );
}
...
function _beans_get_image_alt( $image_id ) {
	...
	if ( ! $image_alt ) {
		return __( 'Sorry, no description was given for this image.', 'beans' );
	}
