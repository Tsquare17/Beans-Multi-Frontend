...
namespace Beans\Framework\API\Fields;
...
function beans_get_fields( $context, $section = false ) {
	...
	if ( ! class_exists( __NAMESPACE__ . '\_Beans_Fields' ) ) {
		return;
	}
...
function beans_field( array $field ) {
	...
	if ( $group_field_type ) {
		HTML\beans_open_markup_e(
			...
			HTML\beans_close_markup_e( 'beans_field_group_legend', 'legend', $field );
	}
	...
	foreach ( $fields as $single_field ) {
		...
		HTML\beans_close_markup_e( 'beans_field[_' . $single_field['id'] . ']', 'div', $single_field );
	}

	if ( $group_field_type ) {
		HTML\beans_close_markup_e( 'beans_field_group_fieldset', 'fieldset', $field );
	}
	HTML\beans_close_markup_e( 'beans_field_inside', 'div', $fields );
	HTML\beans_close_markup_e( 'beans_field_wrap', 'div', $field );
}
...
function _beans_pre_standardize_fields( array $fields ) {
	...
	foreach ( $fields as $field ) {
		...
		if ( 'group' === Utilities\beans_get( 'type', $field ) ) {
