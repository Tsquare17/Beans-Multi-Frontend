...
namespace Beans\Framework\API\HTML;
...
function beans_output( $id, $output ) {
	...
	$output = Filters\beans_apply_filters(...$args);
...
function beans_output_e( $id, $output ) {
	$args = func_get_args();
	echo beans_output(...$args); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in beans_output.
}
...
function beans_open_markup( $id, $tag, $attributes = array() ) {
	...
	// Filter the tag.
	$tag = Filters\beans_apply_filters(...$args);
	...
	$output  = Actions\_beans_render_action(...$args);

	// Build the opening tag when tag is available.
	if ( $tag ) {
		$output .= '<' . esc_attr( $tag ) . ' ' . beans_add_attributes(...$attributes_args) . ( _beans_is_html_dev_mode() ? ' data-markup-id="' . esc_attr( $id ) . '"' : null ) . ( $_beans_is_selfclose_markup ? '/' : '' ) . '>';
	}
...
function beans_open_markup_e( $id, $tag, $attributes = array() ) {
	$args = func_get_args();
	echo beans_open_markup(...$args); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in beans_open_markup().
}
...
function beans_selfclose_markup_e( $id, $tag, $attributes = array() ) {
	$args = func_get_args();
	echo beans_selfclose_markup(...$args); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in beans_open_markup().
}
...
function beans_close_markup( $id, $tag ) {
	// Filter the tag.
	$tag = Filters\beans_apply_filters( $id . '_markup', $tag );
...
function beans_close_markup_e( $id, $tag ) {
	$args = func_get_args();
	echo beans_close_markup(...$args); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in beans_close_markup().
}
...
function beans_wrap_markup( $id, $new_id, $tag, $attributes = array() ) {
	...
	Actions\_beans_add_anonymous_action( $id . '_before_markup', array( 'beans_open_markup', $args ), 9999 );

	unset( $args[3] );

	Actions\_beans_add_anonymous_action( $id . '_after_markup', array( 'beans_close_markup', $args ), 1 );
...
function beans_wrap_inner_markup( $id, $new_id, $tag, $attributes = array() ) {
	...
	Actions\_beans_add_anonymous_action( $id . '_prepend_markup', array( 'beans_open_markup', $args ), 1 );

	unset( $args[3] );

	Actions\_beans_add_anonymous_action( $id . '_append_markup', array( 'beans_close_markup', $args ), 9999 );
...
function beans_add_attributes( $id, $attributes = array() ) {
	...
	$attributes = Filters\beans_apply_filters(...$args);

	return Utilities\beans_esc_attributes( $attributes );
}
