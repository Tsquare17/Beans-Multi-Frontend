...
namespace Beans\Framework\API\Post_Meta;
...
function beans_register_post_meta( array $fields, $conditions, $section, $args = array() ) {
	 ...
	 */
	$fields = apply_filters( "beans_post_meta_fields_{$section}", Fields\_beans_pre_standardize_fields( $fields ) );
	...
	if ( ! Fields\beans_register_fields( $fields, 'post_meta', $section ) ) {
		return false;
	}
...
function _beans_is_post_meta_conditions( $conditions ) {
	...
	if ( false !== stripos( $_SERVER['REQUEST_URI'], 'post-new.php' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput -- We are checking if the request path includes a certain sting in an admin request. No other processing is done.
		$current_post_type = Utilities\beans_get( 'post_type' );
		...
		$id_get = Utilities\beans_get( 'post' );
		// Try to get ID from $_POST.
		$id_post = Utilities\beans_post( 'post_ID' );
...
add_action( 'admin_print_footer_scripts', __NAMESPACE__ . '\_beans_post_meta_page_template_reload' );
/**
