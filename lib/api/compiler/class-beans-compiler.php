...
final class _Beans_Compiler {
	...
	public function set_fragments() {
		...
		$added_fragments = Utilities\beans_get( $this->config['id'], $_beans_compiler_added_fragments[ $this->config['format'] ] );
	...
	public function get_internal_content() {
		...
		if ( ! file_exists( $fragment ) ) {
			...
			$fragment = Utilities\beans_url_to_path( $fragment );
	...
	public function add_content_media_query( $content ) {
		...
		return sprintf(
			"@media %s {\n%s\n}\n",
			Utilities\beans_get( 'beans_compiler_media_query', wp_parse_args( $query ) ),
			$content
		);
	}
	...
	public function format_content( $content ) {
		...
		if ( 'style' === $this->config['type'] ) {
			...
			if ( 'less' === $this->config['format'] ) {
				...
				if ( ! class_exists( __NAMESPACE__ . '\Beans_Lessc' ) ) {
		...
		if ( 'script' === $this->config['type'] && ! _beans_is_compiler_dev_mode() && $this->config['minify_js'] ) {
			...
			if ( ! class_exists( __NAMESPACE__ . '\JSMin' ) ) {
	...
	public function replace_css_url_callback( $matches ) {
		...
		if ( Utilities\_beans_is_uri( $matches[1] ) ) {
			return $matches[0];
		}
		...
		$url = Utilities\beans_path_to_url( trailingslashit( $base ) . ltrim( end( $paths ), '/\\' ) );
	...
	private function remove_modified_files( $hash, $filemtime_hash ) {
		$items = Utilities\beans_scandir( $this->dir );
	...
	private function kill() {
		...
		// Send report if set.
		if ( Utilities\beans_get( 'beans_send_compiler_report' ) ) {
			$this->report();
		}

		$html = HTML\beans_output(
