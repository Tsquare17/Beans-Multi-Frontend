...
function beans_flush_compiler( $id, $file_format = false, $admin = false ) {
	...
	if ( ! $file_format ) {
		Utilities\beans_remove_dir( $dir );

		return;
	}

	// Remove only the specified file format.
	foreach ( Utilities\beans_scandir( $dir ) as $item ) {

		if ( Utilities\beans_str_ends_with( $item, ".{$file_format}" ) ) {
...
add_action( 'beans_loaded_api_component_compiler', __NAMESPACE__ . '\beans_add_compiler_options_to_settings' );
/**
...
function beans_add_compiler_options_to_settings() {
	...
	if ( ! class_exists( __NAMESPACE__ . '\_Beans_Compiler_Options' ) ) {
		return;
	}
...
add_action( 'beans_loaded_api_component_compiler', __NAMESPACE__ . '\beans_add_page_assets_compiler' );
/**
...
function beans_add_page_assets_compiler() {
	...
	if ( ! class_exists( __NAMESPACE__ . '\_Beans_Page_Compiler' ) ) {
		return;
	}
