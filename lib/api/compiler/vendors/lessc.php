...
class Beans_Lessc {
	...
	protected function lib_pow($args) {
		...
		return $this->assertNumber($base) ** $this->assertNumber($exp);
	}
	...
	protected function assertColor($value, $error = 'expected color value') {
		...
		if ($color === null) $this->throwError($error);
		return $color;
	}
	...
	protected function reduce($value, $forExpression = false) {
		...
		case 'function':
			...
			$f = $this->libFunctions[$name] ?? array($this, 'lib_' . $name);
...
// No changes, pass back the structure
	...
	protected function newFormatter() {
		$className = __NAMESPACE__ . '\beans_lessc_formatter_lessjs';
		if (!empty($this->formatterName)) {
			if (!is_string($this->formatterName))
				return $this->formatterName;
			$className = __NAMESPACE__ . "\beans_lessc_formatter_$this->formatterName";
		}
...
class beans_lessc_parser {
	...
	public function __construct($lessc, $sourceName = null) {
		...
		if (!self::$operatorString) {
			self::$operatorString =
				'('.implode('|', array_map(array(__NAMESPACE__ . '\Beans_Lessc', 'preg_quote'),
	...
	protected function parseChunk() {
		...
		if ($this->literal('@', false)) {
			...
			if ($this->literal('@media')) {
				// possible error, first condition will always return true
				if (($this->mediaQueryList($mediaQueries) || true)
					&& $this->literal('{'))
				{
					$media = $this->pushSpecialBlock('media');
					$media->queries = $mediaQueries ?? array();
					return true;
				}
	...
	protected function isDirective($dirname, $directives) {
		...
		$pattern = implode('|',
			array_map(array(__NAMESPACE__ . '\Beans_Lessc', 'preg_quote'), $directives));
	...
	protected function openString($end, &$out, $nestingOpen=null, $rejectStrs = null) {
		...
		$stop = array("'", '"', '@{', $end);
		$stop = array_map(array(__NAMESPACE__ . '\Beans_Lessc', 'preg_quote'), $stop);
