...
final class _Beans_Page_Compiler {
	...
	private function do_not_compile_styles() {
		return ! API\beans_get_component_support( 'wp_styles_compiler' ) || ! get_option( 'beans_compile_all_styles', false ) || _beans_is_compiler_dev_mode();
	}
	...
	private function do_not_compile_scripts() {
		return ! API\beans_get_component_support( 'wp_scripts_compiler' ) || ! get_option( 'beans_compile_all_scripts', false ) || _beans_is_compiler_dev_mode();
	}
	...
	private function compile_enqueued( $type, $dependencies = '' ) {
		$assets = Utilities\beans_get( "wp_{$type}s", $GLOBALS );
		...
		foreach ( $dependencies as $handle ) {
			...
			$asset = Utilities\beans_get( $handle, $assets->registered );
	...
	public function dequeue_scripts() {
		...
		foreach ( $this->dequeued_scripts as $handle => $src ) {
			$script = Utilities\beans_get( $handle, $wp_scripts->registered );
