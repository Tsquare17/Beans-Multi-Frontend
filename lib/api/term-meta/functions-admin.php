...
function beans_register_term_meta( array $fields, $taxonomies, $section ) {
	 ...
	 */
	$fields = apply_filters( "beans_term_meta_fields_{$section}", Fields\_beans_pre_standardize_fields( $fields ) );
	...
	if ( ! Fields\beans_register_fields( $fields, 'term_meta', $section ) ) {
		return false;
	}
...
function _beans_is_admin_term( $taxonomies ) {
	...
	$taxonomy = Utilities\beans_get_or_post( 'taxonomy' );
