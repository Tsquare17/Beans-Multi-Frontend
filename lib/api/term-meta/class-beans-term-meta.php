...
final class _Beans_Term_Meta {
	...
	public function __construct( $section ) {
		...
		$this->do_once();
		add_action( Utilities\beans_get( 'taxonomy' ) . '_edit_form_fields', array( $this, 'render_fields' ) );
	}
	...
	private function do_once() {
		...
		add_action( Utilities\beans_get( 'taxonomy' ) . '_edit_form', array( $this, 'render_nonce' ) );
	...
	public function render_fields() {
		Actions\beans_remove_action( 'beans_field_label' );
		Actions\beans_modify_action_hook( 'beans_field_description', 'beans_field_wrap_after_markup' );
		HTML\beans_modify_markup( 'beans_field_description', 'p' );
		HTML\beans_add_attribute( 'beans_field_description', 'class', 'description' );

		foreach ( Fields\beans_get_fields( 'term_meta', $this->section ) as $field ) {
	...
	public function save( $term_id ) {
		...
		if ( Utilities\_beans_doing_ajax() ) {
			return $term_id;
		}

		if ( ! wp_verify_nonce( Utilities\beans_post( 'beans_term_meta_nonce' ), 'beans_term_meta_nonce' ) ) {
			return $term_id;
		}

		$fields = Utilities\beans_post( 'beans_fields' );
