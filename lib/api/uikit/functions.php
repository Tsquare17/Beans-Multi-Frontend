...
function beans_uikit_enqueue_components( $components, $type = 'core', $autoload = true ) {
	...
	$_beans_uikit_enqueued_items['components'][ $type ] = Utilities\beans_join_arrays_clean( (array) $_beans_uikit_enqueued_items['components'][ $type ], (array) $components );
}
...
function beans_uikit_register_theme( $id, $path ) {
	...
	if ( Utilities\beans_get( $id, $_beans_uikit_registered_items['themes'] ) ) {
		return true;
	}
	...
	if ( Utilities\beans_str_starts_with( $path, 'http' ) ) {
		$path = Utilities\beans_url_to_path( $path );
	}
...
function _beans_uikit_get_registered_theme( $id ) {
	...
	return Utilities\beans_get( $id, $_beans_uikit_registered_items['themes'], false );
}

add_action( 'wp_enqueue_scripts', __NAMESPACE__ . '\_beans_uikit_enqueue_assets', 7 );
/**
...
add_action( 'admin_enqueue_scripts', __NAMESPACE__ . '\_beans_uikit_enqueue_admin_assets', 7 );
/**
