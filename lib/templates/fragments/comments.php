...
beans_add_smart_action( 'beans_comments_list_before_markup', __NAMESPACE__ . '\beans_comments_title' );
/**
...
beans_add_smart_action( 'beans_comment_header', __NAMESPACE__ . '\beans_comment_avatar', 5 );
/**
...
beans_add_smart_action( 'beans_comment_header', __NAMESPACE__ . '\beans_comment_author' );
/**
...
beans_add_smart_action( 'beans_comment_title_append_markup', __NAMESPACE__ . '\beans_comment_badges' );
/**
...
function beans_comment_badges() {
	...
	if ( 'trackback' === $comment->comment_type ) {
		beans_open_markup_e( 'beans_trackback_badge', 'span', array( 'class' => 'uk-badge uk-margin-small-left' ) );
			...
			beans_output_e( 'beans_trackback_text', esc_html__( 'Trackback', 'beans' ) );
	...
	if ( 'pingback' === $comment->comment_type ) {
		beans_open_markup_e( 'beans_pingback_badge', 'span', array( 'class' => 'uk-badge uk-margin-small-left' ) );
			...
			beans_output_e( 'beans_pingback_text', esc_html__( 'Pingback', 'beans' ) );
	...
	if ( '0' === $comment->comment_approved ) {
		beans_open_markup_e( 'beans_moderation_badge', 'span', array( 'class' => 'uk-badge uk-margin-small-left uk-badge-warning' ) );
			...
			beans_output_e( 'beans_moderation_text', esc_html__( 'Awaiting Moderation', 'beans' ) );
	...
	if ( user_can( $comment->user_id, 'moderate_comments' ) ) {
		beans_open_markup_e( 'beans_moderator_badge', 'span', array( 'class' => 'uk-badge uk-margin-small-left' ) );
			...
			beans_output_e( 'beans_moderator_text', esc_html__( 'Moderator', 'beans' ) );

		beans_close_markup_e( 'beans_moderator_badge', 'span' );
	}
}

beans_add_smart_action( 'beans_comment_header', __NAMESPACE__ . '\beans_comment_metadata', 15 );
/**
...
beans_add_smart_action( 'beans_comment_content', __NAMESPACE__ . '\beans_comment_content' );
/**
...
beans_add_smart_action( 'beans_comment_content', __NAMESPACE__ . '\beans_comment_links', 15 );
/**
	...
	if ( current_user_can( 'moderate_comments' ) ) :
		beans_open_markup_e( 'beans_comment_item[_edit]', 'li' );
			...
			beans_open_markup_e(
				...
				beans_output_e( 'beans_comment_edit_text', esc_html__( 'Edit', 'beans' ) );
		...
		beans_open_markup_e( 'beans_comment_item[_link]', 'li' );
			...
			beans_open_markup_e(
				...
				beans_output_e( 'beans_comment_link_text', esc_html__( 'Link', 'beans' ) );
...
beans_add_smart_action( 'beans_no_comment', __NAMESPACE__ . '\beans_no_comment' );
/**
...
function beans_no_comment() {
	beans_open_markup_e( 'beans_no_comment', 'p', 'class=uk-text-muted' );
		...
		beans_output_e( 'beans_no_comment_text', esc_html__( 'No comment yet, add your voice below!', 'beans' ) );

	beans_close_markup_e( 'beans_no_comment', 'p' );
}

beans_add_smart_action( 'beans_comments_closed', __NAMESPACE__ . '\beans_comments_closed' );
/**
...
function beans_comments_closed() {
	beans_open_markup_e( 'beans_comments_closed', 'p', array( 'class' => 'uk-alert uk-alert-warning uk-margin-bottom-remove' ) );
		...
		beans_output_e( 'beans_comments_closed_text', esc_html__( 'Comments are closed for this article!', 'beans' ) );

	beans_close_markup_e( 'beans_comments_closed', 'p' );
}

beans_add_smart_action( 'beans_comments_list_after_markup', __NAMESPACE__ . '\beans_comments_navigation' );
/**
...
function beans_comments_navigation() {
	...
	beans_open_markup_e(
		...
		array(
			'role'       => 'navigation',
			'aria-label' => esc_attr__( 'Comments Pagination Navigation', 'beans' ),
		)
	);
		...
		if ( get_previous_comments_link() ) {
			beans_open_markup_e( 'beans_comments_navigation_item[_previous]', 'li', array( 'class' => 'uk-pagination-previous' ) );
				...
				echo get_previous_comments_link( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Echoes HTML output.
					$previous_icon . beans_output( 'beans_previous_text[_comments_navigation]', __( 'Previous Comments', 'beans' ) )
				);
		...
		if ( get_next_comments_link() ) {
			beans_open_markup_e( 'beans_comments_navigation_item[_next]', 'li', array( 'class' => 'uk-pagination-next' ) );
				...
				echo get_next_comments_link( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Echoes HTML output.
					beans_output( 'beans_next_text[_comments_navigation]', __( 'Next Comments ', 'beans' ) ) . $next_icon
				);
...
beans_add_smart_action( 'beans_after_open_comments', __NAMESPACE__ . '\beans_comment_form_divider' );
/**
...
beans_add_smart_action( 'beans_after_open_comments', __NAMESPACE__ . '\beans_comment_form' );
/**
...
function beans_comment_form() {
	$output = beans_open_markup( 'beans_comment_form_wrap', 'div', array( 'class' => 'uk-form tm-comment-form-wrap' ) );
		...
		$output .= beans_render_function( 'comment_form', array( 'title_reply' => beans_output( 'beans_comment_form_title_text', __( 'Add a Comment', 'beans' ) ) ) );
	...
	$submit = beans_open_markup(
		...
		$submit .= beans_output( 'beans_comment_form_submit_text', esc_html__( 'Post Comment', 'beans' ) );
...
// Filter.
beans_add_smart_action( 'cancel_comment_reply_link', __NAMESPACE__ . '\beans_comment_cancel_reply_link', 10, 3 );
/**
...
// Filter.
beans_add_smart_action( 'comment_form_field_comment', __NAMESPACE__ . '\beans_comment_form_comment' );
/**
...
function beans_comment_form_comment() {
	...
	if ( beans_apply_filters( 'beans_comment_form_legend[_comment]', true ) ) {
		...
		$output .= beans_open_markup( 'beans_comment_form_legend[_comment]', 'legend' );
			...
			$output .= beans_output( 'beans_comment_form_legend_text[_comment]', esc_html__( 'Comment *', 'beans' ) );
...
beans_add_smart_action( 'comment_form_before_fields', __NAMESPACE__ . '\beans_comment_before_fields', 9999 );
/**
...
// Filter.
beans_add_smart_action( 'comment_form_default_fields', __NAMESPACE__ . '\beans_comment_form_fields' );
/**
	...
	if ( isset( $fields['author'] ) ) {
		...
		if ( beans_apply_filters( 'beans_comment_form_legend[_name]', true ) ) {
			$author .= beans_open_markup( 'beans_comment_form_legend[_name]', 'legend' );
				...
				$author .= beans_output( 'beans_comment_form_legend_text[_name]', esc_html__( 'Name *', 'beans' ) );
		...
		if ( beans_apply_filters( 'beans_comment_form_legend[_email]', true ) ) {
			$email .= beans_open_markup( 'beans_comment_form_legend[_email]', 'legend' );
				...
				$email .= beans_output(
					...
					sprintf( __( 'Email %s', 'beans' ), ( get_option( 'require_name_email' ) ? ' *' : '' ) )
				);
	...
	if ( isset( $fields['url'] ) ) {
		...
		if ( beans_apply_filters( 'beans_comment_form_legend[_url]', true ) ) {
			$url .= beans_open_markup( 'beans_comment_form_legend', 'legend' );
				...
				$url .= beans_output( 'beans_comment_form_legend_text[_url]', esc_html__( 'Website', 'beans' ) );
...
beans_add_smart_action( 'comment_form_after_fields', __NAMESPACE__ . '\beans_comment_form_after_fields', 3 );
/**
