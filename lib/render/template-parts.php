...
Actions\beans_add_smart_action( 'beans_load_document', __NAMESPACE__ . '\beans_header_template', 5 );
/**
...
Actions\beans_add_smart_action( 'beans_site_prepend_markup', __NAMESPACE__ . '\beans_header_partial_template' );
/**
...
Actions\beans_add_smart_action( 'beans_load_document', __NAMESPACE__ . '\beans_content_template' );
/**
...
Actions\beans_add_smart_action( 'beans_content', __NAMESPACE__ . '\beans_loop_template' );
/**
...
function beans_loop_template( $id = false ) {
	...
	$_has_filter = Filters\beans_has_filters( "beans_loop_query_args[_{$id}]" );
	...
	if ( $_has_filter ) {
		 ...
		 * @since 1.0.0
		 */
		$args     = Filters\beans_apply_filters( "beans_loop_query_args[_{$id}]", false );
...
Actions\beans_add_smart_action( 'beans_post_after_markup', __NAMESPACE__ . '\beans_comments_template', 15 );
/**
...
function beans_comments_template() {
	...
	$shortcircuit_conditions = array(
		Utilities\beans_get( 'ID', $post ) && ! ( comments_open() || get_comments_number() ),
		! post_type_supports( Utilities\beans_get( 'post_type', $post ), 'comments' ),
	);
...
Actions\beans_add_smart_action( 'beans_comment', __NAMESPACE__ . '\beans_comment_template' );
/**
...
Actions\beans_add_smart_action( 'beans_widget_area', __NAMESPACE__ . '\beans_widget_area_template' );
/**
...
Actions\beans_add_smart_action( 'beans_primary_after_markup', __NAMESPACE__ . '\beans_sidebar_primary_template' );
/**
...
function beans_sidebar_primary_template() {
	...
	if ( false === stripos( Layout\beans_get_layout(), 'sp' ) || ! Widgets\beans_has_widget_area( 'sidebar_primary' ) ) {
		return;
	}
...
Actions\beans_add_smart_action( 'beans_primary_after_markup', __NAMESPACE__ . '\beans_sidebar_secondary_template' );
/**
...
function beans_sidebar_secondary_template() {
	...
	if ( false === stripos( Layout\beans_get_layout(), 'ss' ) || ! Widgets\beans_has_widget_area( 'sidebar_secondary' ) ) {
		return;
	}
...
Actions\beans_add_smart_action( 'beans_site_append_markup', __NAMESPACE__ . '\beans_footer_partial_template' );
/**
...
Actions\beans_add_smart_action( 'beans_load_document', __NAMESPACE__ . '\beans_footer_template' );
/**
