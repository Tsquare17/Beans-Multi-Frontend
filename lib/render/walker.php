...
class _Beans_Walker_Nav_Menu extends \Walker_Nav_Menu {
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		if ( 'navbar' === $type && $args->beans_start_level === $depth ) {
			...
			// Open sub_menu wrap.
			$output .= HTML\beans_open_markup( "beans_sub_menu_wrap[_{$type}]{$location_subfilter}", 'div', 'class=uk-dropdown uk-dropdown-navbar', $depth, $args );
		}
		...
		// Open sub_menu.
		$output .= HTML\beans_open_markup( "beans_sub_menu[_{$type}]{$location_subfilter}", 'ul', $attr, $depth, $args );
	}
	...
	public function end_lvl( &$output, $depth = 0, $args = array() ) {
		...
		// Close sub_menu.
		$output .= HTML\beans_close_markup( "beans_sub_menu[_{$type}]{$location_subfilter}", 'ul' );

		// Close sub_menu wrap.
		if ( 'navbar' === $type && $args->beans_start_level === $depth ) {
			$output .= HTML\beans_close_markup( "beans_sub_menu_wrap[_{$type}]{$location_subfilter}", 'div', $depth, $args );
		}
	}
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( $args->beans_start_level === $depth && in_array( 'menu-item-has-children', $classes, true ) ) {
			...
			if ( Utilities\beans_get( 'beans_type', $args ) === 'navbar' ) {
		...
		$output .= HTML\beans_open_markup( "beans_menu_item[_{$item_id}]", 'li', $item_attr, $item, $depth, $args );

			$item_output = $args->before;

				$item_output .= HTML\beans_open_markup( "beans_menu_item_link[_{$item_id}]", 'a', $link_attr, $item, $depth, $args );

					$item_output .= HTML\beans_output( "beans_menu_item_text[_{$item_id}]", $args->link_before . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Used inside method scope.

		if ( isset( $child_indicator ) ) {
			$item_output .= HTML\beans_open_markup( "beans_menu_item_child_indicator[_{$item_id}]", 'i', array( 'class' => 'uk-icon-caret-down uk-margin-small-left' ), $item, $depth, $args );
			$item_output .= HTML\beans_close_markup( "beans_menu_item_child_indicator[_{$item_id}]", 'i', $item, $depth, $args );
		}

				$item_output .= HTML\beans_close_markup( "beans_menu_item_link[_{$item_id}]", 'a', $link_attr, $item, $depth, $args );
	...
	public function end_el( &$output, $item, $depth = 0, $args = array() ) {
		...
		$item_id = $item->ID;
		$output .= HTML\beans_close_markup( "beans_menu_item[_{$item_id}]", 'li', $item, $depth, $args );
	}
}
