...
class Tests_BeansRegisterWPCustomizeOptions extends WP_Customize_Test_Case {
	...
	public function test_should_register_fields() {
		...
		foreach ( $test_data['fields'] as $index => $field ) {
			...
			$this->assertSame( $expected, $registered['wp_customize']['beans-test-customizer'][ $index ] );
		}
	}
	...
	public function test_should_add_section_settings_and_controls() {
		...
		$this->assertNotEmpty( $this->wp_customize->get_section( 'beans-test-customizer' ) );
	...
	public function test_should_add_section_and_settings_for_grouped_fields() {
		...
		$this->assertEmpty( $this->wp_customize->get_section( 'beans-test-customizer' ) );
		$this->assertNotEmpty( $this->wp_customize->get_section( 'beans-group-test-customizer' ) );
